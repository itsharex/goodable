...
export function ServiceSettings({ projectId, onOpenGlobalSettings }: ServiceSettingsProps) {
  ...
  const [projectType, setProjectType] = useState<string>('nextjs');
  ...
  useEffect(() => {
    ...
    const loadProjectType = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setProjectType(project.projectType || 'nextjs');
        }
      } catch (error) {
    ...
    loadProjectType();
  }, [loadServiceConnections, checkTokens, projectId]);
  ...
  return (
    <>
      {showDeployPage ? (
        <AliyunDeployPage
          projectId={projectId}
          onClose={() => setShowDeployPage(false)}
        />
      ) : (
              ...
              <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 justify-between">
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 sm:flex-shrink-0 w-full sm:w-auto sm:justify-end">
                    {service.connected ? (
                      <>
                        {service.id === 'aliyun' && projectType === 'python-fastapi' && (
                          <button
                            onClick={() => setShowDeployPage(true)}
                            className="px-4 py-2 text-sm rounded-xl text-blue-600 hover:text-blue-700 border border-transparent hover:border-blue-200 hover:bg-blue-50 transition whitespace-nowrap"
                          >
                            部署
                          </button>
                        )}
